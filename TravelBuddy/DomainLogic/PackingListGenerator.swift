...
struct PackingListGenerator {
	...
	static func generatePackingList(for trip: Trip) -> [PackItem] {
		...
		for transportType in trip.transportTypesEnum {
			...
			case .plane: items.append(contentsOf: airTravelItems())
			case .car: items.append(contentsOf: carTravelItems())
			case .train: items.append(contentsOf: trainTravelItems())
			case .ship: items.append(contentsOf: shipTravelItems())
			default: break
			}
		}
		
		// Add items based on accommodation
		switch trip.accommodationTypeEnum {
		case .hotel: items.append(contentsOf: hotelItems())
		case .camping: items.append(contentsOf: campingItems())
		case .apartment, .airbnb: items.append(contentsOf: apartmentItems())
		case .hostels: items.append(contentsOf: hostelItems())
		case .friends: items.append(contentsOf: stayingWithFriendsItems())
		}
		
		// Add items based on activities
		for activity in trip.activitiesEnum {
			switch activity {
			case .swimming: items.append(contentsOf: swimmingItems())
			case .hiking: items.append(contentsOf: hikingItems())
			case .business: items.append(contentsOf: businessItems())
			case .beach: items.append(contentsOf: beachItems())
			case .sports: items.append(contentsOf: sportsItems())
			case .skiing: items.append(contentsOf: skiingItems())
			case .sightseeing: items.append(contentsOf: sightseeingItems())
			case .relaxing: items.append(contentsOf: relaxingItems())
			}
		}
		...
		case .hot: items.append(contentsOf: hotWeatherItems())
		case .warm: items.append(contentsOf: warmWeatherItems())
		case .cold: items.append(contentsOf: coldWeatherItems())
		case .cool: items.append(contentsOf: coolWeatherItems())
		default: items.append(contentsOf: moderateWeatherItems())
		}
